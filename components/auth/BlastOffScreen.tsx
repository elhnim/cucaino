...
// The "Blast-off" rocket loading visual: floating clouds, twinkles, rotating
// fun messages and an easing progress bar. Pure presentation — used by
// PostLoginLoader (which adds real preloading + navigation) and by
// app/select-kid/loading.tsx (streamed instantly while the server renders).
...
export default function BlastOffScreen({
  ready = false,
  prefetch = false,
}: {
  ready?: boolean;
  ...
  // while the rocket entertains, warm up every kid's screens so the rest of
  // the app opens instantly afterwards
  useEffect(() => {
    if (!prefetch || prefetched.current) return;
    prefetched.current = true;
    void prefetchFamilyRoutes(router);
  }, [prefetch, router]);
    ...
    >
      ...
      <div className="mt-5 w-64 max-w-[70vw] h-5 bg-white/30 border-[3px] border-white rounded-full overflow-hidden" style={{ boxShadow: "0 4px 0 rgba(46,16,101,.18)" }}>
        <div
          ...
          style={{ width: `${Math.round(ready ? 100 : pct)}%` }}
        />
      </div>
